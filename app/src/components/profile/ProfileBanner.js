...
const ProfileBanner = (props) => {
    ...
    const handleFileChange = async event => {
        ...
        if(typeof data === 'object') {
            ...
            .then(res=> res.json())
            .then(res=> MessageCtx.userProvider.banner = processedBanner) // need to exibit later a modal saying: "it's done!"
    ...
    return(
        <Box component='button' onClick={handleClick} sx={{position:'absolute', p:0, m:0, border:'none', borderRadius:3}}>
            ...
            {/* This is quite the tenary, so what happens is that we check if the banner state exists */}
